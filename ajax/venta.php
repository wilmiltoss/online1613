...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idventa)){
			...
			$num_comprobante,$fecha_hora,$impuesto,$total_venta,$_POST["idarticulo"],
	...
	case 'listar':
		$rspta=$venta->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			...
 			$data[]=array(
				 "0"=>(($reg->estado=='Aceptado')?'<button class="btn btn-warning" onclick="mostrar('.$reg->idventa.')">
					 <i class="fa fa-eye"></i></button>'.' <button class="btn btn-danger" onclick="anular('.$reg->idventa.')">
					 <i class="fa fa-close"></i></button>':
					 '<button class="btn btn-warning" onclick="mostrar('.$reg->idventa.')"><i class="fa fa-eye">
					 </i></button>').'<a target="_blank" href="'.$url.$reg->idventa.'"> <button class="btn btn-info">
